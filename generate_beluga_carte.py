...
def get_official_adress_from_coordinates (lat, long):
    ...
    URL_GOUV = 'https://api-adresse.data.gouv.fr/reverse/?lon=%f&lat=%f&type=street&limit=1' % (long,lat)
...
parser = argparse.ArgumentParser(description='Generateur de la carte des contacts Beluga, pour faciliter les mises en relation covoiturage.')
parser.add_argument('--input', dest='xls_input_file', default='exemple_contacts.xlsx', 
                        help='Nom du fichier contenant les contacts')


parser.add_argument('--output', dest='html_output_file', default='carte_contact.html', 
                        help='Fichier html de sortie contenant la carte')

# Analyser les arguments à partir de la ligne de commande
args = parser.parse_args()
print ("generation de la carte a partir du fichier source '%s'\nLa carte sera générée dans le fichier '%s'" %  (args.xls_input_file,args.html_output_file))
...
title_html = '<h3 align="center" style="font-size:20px"><b>Beluga ! </b> adresse %s</h3>' % params['location']
...
for index, row in df.iterrows():
    ...
    marker = folium.Marker(
        location=get_coordinates(row['adresse floute']),  # Vous devez remplacer ces coordonnées par celles de chaque lieu de rencontre
